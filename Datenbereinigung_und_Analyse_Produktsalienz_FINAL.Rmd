---
title: "Datenbereinigung und Analyse – Produktsalienz"
author: "Gruppe Salienz"
output:
  pdf_document:
    toc: true
...
knitr::opts_chunk$set(
  ...
  fig.width = 6.4,
  fig.height = 4.2,
  out.width = if (knitr::is_latex_output()) "0.82\\linewidth" else "88%",
  collapse = FALSE
)

# Begrenzte Konsolenbreite sorgt auch bei textbasierten Ergebnisobjekten
# für einen sauberen Zeilenumbruch in der Ausgabe.
options(width = 78)
options(knitr.kable.NA = "")

# Reine Darstellungsfunktion: verändert keine Berechnungen oder Ergebnisobjekte.
# Breite Tabellen werden ausschließlich für die Ausgabe in mehrere schmale
# Teil-Tabellen zerlegt. Die zugrunde liegenden Ergebnisobjekte bleiben unverändert.
drucke_tabelle <- function(tab, titel = NULL, digits = 3) {
  # Einzeilige, sehr breite Resultate sind als Kennwert-Wert-Tabelle
  # deutlich besser lesbar und können nicht über den Seitenrand laufen.
  if (nrow(tab) == 1 && ncol(tab) > 5) {
    tab <- data.frame(
      Kennwert = names(tab),
      Wert = vapply(tab[1, , drop = FALSE], function(z) {
        paste(z, collapse = ", ")
      }, character(1)),
      row.names = NULL,
      check.names = FALSE
  ...
  # Pandoc-Pipe-Tabellen werden im PDF mit berechneten Spaltenbreiten gesetzt.
  # Bei mehr als fünf Spalten wird die Tabelle zusätzlich in Teilblöcke geteilt.
  if (ncol(tab) <= 5) {
    print(knitr::kable(
      tab,
      format = "pipe",
  ...
  id_n <- if (ncol(tab) >= 8) 2 else 1
  id_cols <- seq_len(id_n)
  rest_cols <- setdiff(seq_len(ncol(tab)), id_cols)
  pro_teil <- max(1, 5 - id_n)
  gruppen <- split(
    rest_cols,
    ceiling(seq_along(rest_cols) / pro_teil)
  )

  for (i in seq_along(gruppen)) {
    teil <- tab[, c(id_cols, gruppen[[i]]), drop = FALSE]
    teil_titel <- if (length(gruppen) == 1 || is.null(titel)) {
      titel
    } else {
      paste0(titel, " - Teil ", i, " von ", length(gruppen))
    }
...
zeige_ergebnis <- function(x, titel = NULL, digits = 3) {
  if (inherits(x, "htest")) {
    tab <- data.frame(
      Kennwert = character(),
      Wert = character(),
    ...
    add <- function(k, v) {
      if (length(v) > 0 && !all(is.na(v))) {
        tab <<- rbind(
          ...
          data.frame(
            Kennwert = k,
            Wert = paste(v, collapse = ", "),
            stringsAsFactors = FALSE
          )
        )
      }
    }
    add("Methode", x$method)
    if (!is.null(x$statistic)) {
      add(
        names(x$statistic) %||% "Teststatistik",
        round(unname(x$statistic), digits)
      )
    }
    if (!is.null(x$parameter)) {
      add(
        names(x$parameter) %||% "Freiheitsgrade",
        round(unname(x$parameter), digits)
      )
    }
    if (!is.null(x$estimate)) {
      add(
        paste(names(x$estimate), collapse = ", "),
        round(unname(x$estimate), digits)
      )
    }
    if (!is.null(x$conf.int)) {
      add(
        "Konfidenzintervall",
        round(unname(x$conf.int), digits)
      )
    }
    add(
      "p-Wert",
    ...
    if (!is.null(x$alternative)) add("Alternative", x$alternative)
  } else if (inherits(x, "afex_aov")) {
    tab <- as.data.frame(afex::nice(x))
  } else if (inherits(x, "emmGrid") || inherits(x, "emm_list")) {
    tab <- as.data.frame(x)
  } else if (is.table(x)) {
    tab <- as.data.frame(x)
  } else if (is.matrix(x)) {
    ...
    if (!is.null(names(x)) && any(nzchar(names(x)))) {
      ...
      tab <- data.frame(Wert = x, row.names = NULL)
    }
  } else {
    versuch <- try(as.data.frame(x), silent = TRUE)
    if (!inherits(versuch, "try-error")) {
      tab <- versuch
    } else {
      tab <- data.frame(
        Ausgabe = capture.output(print(x)),
        stringsAsFactors = FALSE
      )
    }
  }

  drucke_tabelle(tab, titel = titel, digits = digits)
}
...
zeige_wenn <- function(objektname, titel = NULL, digits = 3) {
  if (exists(objektname, inherits = TRUE)) {
    zeige_ergebnis(
      get(objektname, inherits = TRUE),
      titel = titel,
      digits = digits
    )
  }
...
zeige_alpha <- function(x, titel = NULL) {
  if (!is.null(x$total)) {
    zeige_ergebnis(x$total, titel = titel, digits = 3)
  } else {
    zeige_ergebnis(x, titel = titel, digits = 3)
  }
}
```
...
Pakete, Bedingungslabels, Grafik-Theme und die Funktionen zum Speichern der Grafiken werden vorbereitet.


```{r setup-original, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
# Interne Condition-Namen bleiben unverändert, damit Filter, Tests und Kontraste exakt
# gleich funktionieren.
# Für sichtbare Tabellen- und Diagrammbeschriftungen werden ausschließlich die
...
# Seitengerechtes Theme für Boxplots und Dichteplots.
# Es verändert ausschließlich die Darstellung, nicht die zugrunde liegenden Daten.
theme_box_density_gross <- theme_minimal(base_size = 11) +
  theme(
...
speichere_grafik <- function(plot, name = "Grafik") {
  ...
  # Einheitliche, normale Grafikgröße für gespeicherte PNG-Dateien.
  # Die Analyse selbst bleibt unverändert.
  breite <- 8
  hoehe <- 5.5
  
  if (grepl("Dichteplots_der_WSI", sicherer_name)) {
    breite <- 8
    hoehe <- 7
  }
...
```{r setup-kontrolle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  data.frame(Kennwert = "Zeilen im Rohdatensatz vor Headerbereinigung", Wert = nrow(salienz)),
  "Kontrolle des eingelesenen Rohdatensatzes"
)
```
...
## Headerbereinigung und Entfernung der Testteilnehmenden


```{r datenaufbereitung-header, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
# diese haben Matheaufgaben nicht beantwortet  -> dort steht aber kein N/A sondern -77
# wegen unipark
...
```{r datenaufbereitung-header-ergebnis, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  data.frame(Kennwert = "Teilnehmende nach Entfernung der Testteilnehmenden", Wert = nrow(salienz)),
  "Stichprobengröße nach erstem Ausschluss"
)
```

## Demografischer Überblick vor Ausschluss mobiler Geräte


```{r demografie-vor-filter, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Beginn Analyse
# 1. Ziel: Stichproben Demografie aller Teilnehmer - inklusive Handy User - zum
...
```{r demografie-vor-filter-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(alter_plot, "Altersverteilung vor Gerätefilter")
zeige_ergebnis(geschlecht_plot, "Geschlechtsverteilung vor Gerätefilter")
zeige_ergebnis(einkommen_plot, "Einkommensverteilung vor Gerätefilter")
zeige_ergebnis(taetigkeit_plot, "Tätigkeitsverteilung vor Gerätefilter")
```

## Gerätefilter und Antwortgeschwindigkeit


```{r geraete-und-antwortzeit, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
salienz <- salienz %>%
  mutate( Geraetekategorie = case_when(
    ...
    # Extra-Kategorie falls ein browser.id nicht zugeordnet werden kann und dann
    # manuell prüfen
    TRUE ~ "Unklar"))
...
zeige_und_speichere_grafik(
  ggplot(salienz, aes(x = Bearbeitungszeit_Minuten)) +
    ...
    geom_vline(
      xintercept = median(
        salienz$Bearbeitungszeit_Minuten,
        na.rm = TRUE
      ),
      linetype = "dashed",
      linewidth = 0.8
...
# Berechnen der Bearbeitungszeiten der fünf Outfits, die relative timestamps sind
# Zeitstempel
# -> also muss immer der der vorhergehenden Seite abgezogen werden
salienz <- salienz %>%
  mutate(
    Zeit_7698435 =
      as.numeric(relative.timestamp.for.page.7698435) -
      as.numeric(relative.timestamp.for.page.7698434),
    Zeit_7698431 =
      as.numeric(relative.timestamp.for.page.7698431) -
      as.numeric(relative.timestamp.for.page.7698430),
    Zeit_7698433 =
      as.numeric(relative.timestamp.for.page.7698433) -
      as.numeric(relative.timestamp.for.page.7698432),
    Zeit_7698438 =
      as.numeric(relative.timestamp.for.page.7698438) -
      as.numeric(relative.timestamp.for.page.7698437),
    Zeit_7698427 =
      as.numeric(relative.timestamp.for.page.7698427) -
      as.numeric(relative.timestamp.for.page.7698426)
  )
...
```{r geraete-und-antwortzeit-pruefung-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(geraete_plot, "Verteilung der verwendeten Endgeräte")
zeige_ergebnis(seiten_mediane, "Mediane Bearbeitungszeiten der fünf Outfit-Seiten")
zeige_ergebnis(
  table(salienz$Antwortzeit_auffaellig),
  "Auffällige Antwortgeschwindigkeit nach Relative Speed Index"
)
zeige_ergebnis(
  salienz %>%
    filter(Antwortzeit_auffaellig) %>%
    dplyr::select(number, Bearbeitungszeit_Minuten, Relative_Speed_Index, all_of(zeit_spalten)) %>%
    arrange(desc(Relative_Speed_Index)),
  "Teilnehmende mit auffälliger Antwortgeschwindigkeit"
)
```

```{r antwortzeit-ausschluss, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r antwortzeit-ausschluss-ergebnis, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  data.frame(Kennwert = "Verbleibende Teilnehmende nach Antwortzeitfilter", Wert = nrow(salienz)),
  "Stichprobengröße nach Antwortzeitfilter"
)
```

## Suspicion Probe, Variablenaufbereitung und auffällige Antwortmuster


```{r suspicion-variablen-straightlining, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
# Die Spalten 237 bis 241 bleiben aber weil sie die Randomisierungsreihenfolge der
# fünf Outfits enthalten
salienz <- salienz %>%
  dplyr::select(
    -c(
      2:20,
...
straightlining_pro_outfit <- sapply(conditions, function(condition) {
  ...
  # Für jeden Teilnehmer prüfen ob innerhalb dieser Outfit-Seite alle Antworten gleich
  # sind
  apply(
    salienz[, aktuelle_spalten, drop = FALSE],
    1,
    function(antworten) {
      all(length(unique(antworten)) == 1)
    }
  )
})
...
```{r straightlining-ergebnisse, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(table(straightlining_alle_outfits), "Straightlining über alle fünf Outfit-Seiten")
zeige_ergebnis(straightliner_ids, "IDs der identifizierten Straightliner")
```

## Demografie nach automatischen Filtern und manuelle Prüfung


```{r finale-stichprobe-reshape, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
# Jetzt die demografischen Werte vom Verdächtigen mit ID 32 von der
# Antwortmusteranalyse betrachten
salienz %>%
...
# Sein Alter und Suspicion Probe sind sehr suspicious. Grund genug, den Datensatz zu
# entfernen.
...
```{r demografie-nach-filtern-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(alter_plot, "Altersverteilung nach den automatischen Filtern")
zeige_ergebnis(geschlecht_plot, "Geschlechtsverteilung nach den automatischen Filtern")
zeige_ergebnis(einkommen_plot, "Einkommensverteilung nach den automatischen Filtern")
zeige_ergebnis(taetigkeit_plot, "Tätigkeitsverteilung nach den automatischen Filtern")
zeige_ergebnis(
  salienz %>%
    filter(as.character(number) == "32") %>%
    dplyr::select(number, Alter, Geschlecht, Einkommen, Beruf, Suspicion.Probe),
  "Manuell betrachteter Teilnehmer mit ID 32"
)
```

## Finale Stichprobe, Reshaping und Umpolung der Inkonsistenzskala

```{r finale-stichprobe-reshape-fortsetzung, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r finale-stichprobe-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  data.frame(
    Kennwert = c("Finale Teilnehmendenzahl", "Zeilen im Long-Format"),
    Wert = c(n_distinct(salienz_reshaped$number), nrow(salienz_reshaped))
  ),
...
Die internen Konsistenzen werden zunächst über alle Outfits und anschließend getrennt
nach Outfit und Konstrukt geprüft. Danach werden die Skalenmittelwerte gebildet.


```{r reliabilitaet-skalen, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Cronbach Alpha 
...
# 0.90 (raw_alpha) 
# Sehr gute Konsistenz der Skala. Aber potenziell redundante Items. Wir können die
# Skala so verwenden.
...
konstrukte <- list(
  ...
  Kreativitaet = c(
    "Kreativitaet1", "Kreativitaet2", "Kreativitaet3",
    "Kreativitaet4", "Kreativitaet5"
  ),
  Authentizitaet = c(
    "Authentizitaet1", "Authentizitaet2",
    "Authentizitaet3", "Authentizitaet4"
  ),
  SophisticatedTaste = c(
    "SophisticatedTaste1", "SophisticatedTaste2",
    "SophisticatedTaste3", "SophisticatedTaste4"
  )
)
...
salienz_reshaped$Mittelwert_Inkonsistenz <- rowMeans(
  salienz_reshaped[, c(
    "Inkonsistenz1", "Inkonsistenz2", "Inkonsistenz3"
  )],
  na.rm = TRUE
)

# Bilde den Mittelwert zwischen den Interesse Spalten
salienz_reshaped$Mittelwert_Interesse <- rowMeans(
  salienz_reshaped[, c("Interesse1", "Interesse2")],
  na.rm = TRUE
)

# Bilde den Mittelwert zwischen den Liking Spalten
salienz_reshaped$Mittelwert_Liking <- rowMeans(
  salienz_reshaped[, c("Liking1", "Liking2", "Liking3")],
  na.rm = TRUE
)

# Bilde den Mittelwert zwischen den Fluency Spalten
salienz_reshaped$Mittelwert_Fluency <- rowMeans(
  salienz_reshaped[, c(
    "Fluency1", "Fluency2", "Fluency3", "Fluency4"
  )],
  na.rm = TRUE
)

# Bilde den Mittelwert zwischen den Kreativitaet Spalten
salienz_reshaped$Mittelwert_Kreativitaet <- rowMeans(
  salienz_reshaped[, c(
    "Kreativitaet1", "Kreativitaet2", "Kreativitaet3",
    "Kreativitaet4", "Kreativitaet5"
  )],
...
salienz_reshaped$Mittelwert_Authentizitaet <- rowMeans(
  salienz_reshaped[, c(
    "Authentizitaet1", "Authentizitaet2",
    "Authentizitaet3", "Authentizitaet4"
  )],
...
salienz_reshaped$Mittelwert_SophisticatedTaste <- rowMeans(
  salienz_reshaped[, c(
    "SophisticatedTaste1", "SophisticatedTaste2",
    "SophisticatedTaste3", "SophisticatedTaste4"
  )],
...
```{r reliabilitaet-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_alpha(cronbach_alpha_inkonsistenz, "Cronbachs Alpha – Inkonsistenz")
zeige_alpha(cronbach_alpha_interesse, "Cronbachs Alpha – Interesse")
zeige_alpha(cronbach_alpha_liking, "Cronbachs Alpha – Liking")
zeige_alpha(cronbach_alpha_fluency, "Cronbachs Alpha – Fluency")
zeige_alpha(cronbach_alpha_creativity, "Cronbachs Alpha – Kreativität")
zeige_alpha(cronbach_alpha_authentizitaet, "Cronbachs Alpha – Authentizität")
zeige_alpha(cronbach_alpha_sophisticated, "Cronbachs Alpha – Sophisticated Taste")
zeige_alpha(cronbach_alpha_cvpa, "Cronbachs Alpha – CVPA")
zeige_ergebnis(cronbach_pro_outfit, "Cronbachs Alpha nach Outfit und Konstrukt")
```

# Explorative Bewertung der fünf Outfits


```{r explorative-bewertung, echo=TRUE, message=FALSE, warning=FALSE, results='hide', fig.height=6.8}
...
ggsave(
  filename = "Boxplots_Bewertung_aller_Outfits.png",
  plot = boxplots_outfits,
  width = 8,
  height = 7,
  dpi = 300
)
...
# Hauptanalyse: Wahrgenommene stilistische Inkonsistenz (WSI)


## Hilfsfunktionen für die Hauptanalysen


```{r hauptanalyse-hilfsfunktionen, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Data Analysis 
...
plot_box_density <- function(data, farben, subtitle) {
  ...
  boxplot <- ggplot(
    ...
    labs(
      title = stringr::str_wrap(paste("Boxplot der WSI:", subtitle), width = 48),
      x = "Bedingung",
      y = "WSI"
    ) +
    theme_box_density_gross +
  ...
  density_plot <- ggplot(
    ...
    labs(
      title = stringr::str_wrap(paste("Dichteplot der WSI:", subtitle), width = 48),
      x = "WSI",
      y = "Dichte",
      color = "Bedingung"
    ) +
...
plot_differenz <- function(data, subtitle) {
  ...
  density_plot <- ggplot(
    ...
    labs(
      title = stringr::str_wrap(paste("Dichteplot der Differenzwerte:", subtitle), width = 48),
      x = "Differenz der WSI",
      y = "Dichte"
    ) +
    theme_box_density_gross +
...
## Produktgröße: GDS vs. KDS


```{r hauptanalyse-groesse, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r hauptanalyse-groesse-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_groesse, "Deskriptive Kennwerte – Produktgröße")
zeige_ergebnis(shapiro_auto_01, "Shapiro-Wilk-Test der Differenzwerte – Produktgröße")
zeige_wenn("t_test_groesse", "Gerichteter gepaarter t-Test – Produktgröße")
zeige_wenn("wilcoxon_groesse", "Gerichteter Wilcoxon-Vorzeichen-Rang-Test – Produktgröße")
```

## Position: GZS vs. GDS


```{r hauptanalyse-position, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r hauptanalyse-position-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_position, "Deskriptive Kennwerte – Position")
zeige_ergebnis(shapiro_auto_02, "Shapiro-Wilk-Test der Differenzwerte – Position")
zeige_wenn("t_test_position", "Gerichteter gepaarter t-Test – Position")
zeige_wenn("wilcoxon_position", "Gerichteter Wilcoxon-Vorzeichen-Rang-Test – Position")
```

## Stärke (2 Stufen): GDS vs. GDL


```{r hauptanalyse-staerke-zwei, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r hauptanalyse-staerke-zwei-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke_zwei, "Deskriptive Kennwerte – Stärke (2 Stufen)")
zeige_ergebnis(shapiro_auto_03, "Shapiro-Wilk-Test der Differenzwerte – Stärke (2 Stufen)")
zeige_wenn("t_test_staerke", "Gerichteter gepaarter t-Test – Stärke (2 Stufen)")
zeige_wenn("wilcoxon_staerke", "Gerichteter Wilcoxon-Vorzeichen-Rang-Test – Stärke (2 Stufen)")
```

## Stärke (3 Stufen): M vs. GDL vs. GDS


```{r hauptanalyse-staerke-drei, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
  ...
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Repeated-Measures-ANOVA Stärke (3 Stufen)",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r hauptanalyse-staerke-drei-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke, "Deskriptive Kennwerte – Stärke (3 Stufen)")
zeige_ergebnis(shapiro_auto_04, "Shapiro-Wilk-Test der ANOVA-Residuen – Stärke (3 Stufen)")
if (shapiro_auto_04$p.value > 0.05) {
  zeige_ergebnis(anova_staerke, "Repeated-Measures-ANOVA – Stärke (3 Stufen)")
  zeige_ergebnis(as.data.frame(paarweise_staerke), "Paarweise Vergleiche mit Holm-Korrektur")
} else {
  zeige_wenn("friedman_staerke", "Friedman-Test – Stärke (3 Stufen)")
  zeige_ergebnis(paarweise_staerke, "Paarweise Wilcoxon-Tests mit Holm-Korrektur")
}
```

## Schuh-Stärke (2 Stufen): M vs. KDS


```{r hauptanalyse-schuh, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r hauptanalyse-schuh-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_schuh, "Deskriptive Kennwerte – M vs. KDS")
zeige_ergebnis(shapiro_schuh, "Shapiro-Wilk-Test der Differenzwerte – M vs. KDS")
zeige_wenn("t_test_schuh", "Gerichteter gepaarter t-Test – M vs. KDS")
zeige_wenn("wilcoxon_schuh", "Gerichteter Wilcoxon-Vorzeichen-Rang-Test – M vs. KDS")
```

# Power-Analyse und Fehlerwahrscheinlichkeiten


```{r power-analyse, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r power-analyse-tabelle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(power_uebersicht, "Übersicht der Power-Analysen")
```

# Unterschiede nach Geschlecht


## Hilfsfunktionen für die Mixed-ANOVAs


```{r geschlecht-hilfsfunktionen, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
erstelle_boxplot_geschlecht <- function(data, faktor, stufe, subtitle,
  ...
  p <- p +
    scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
    labs(
      title = stringr::str_wrap(paste("WSI nach Geschlecht:", subtitle), width = 48),
      x = "Geschlecht",
      y = "WSI"
    ) +
    theme_box_density_gross +
...
density_residuen <- function(residuen, subtitle, farbig = TRUE, zentriert = TRUE) {
  ...
  p <- p +
    labs(
      title = stringr::str_wrap(paste("Dichteplot der Residuen:", subtitle), width = 48),
      x = "Residuen",
      y = "Dichte"
    ) +
    theme_box_density_gross
...
# Density-Plots dienen nur der Beschreibung. Die Entscheidung zwischen klassischem und
# robustem Test
...
```{r geschlecht-position, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r geschlecht-position-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_position_geschlecht, "Deskriptive Kennwerte – Position × Geschlecht")
zeige_ergebnis(shapiro_auto_05, "Shapiro-Wilk-Test der Residuen – Position × Geschlecht")
zeige_ergebnis(levene_auto_01, "Levene-Test – GDS nach Geschlecht")
zeige_ergebnis(levene_auto_02, "Levene-Test – GZS nach Geschlecht")
if (shapiro_auto_05$p.value > 0.05 &&
    levene_auto_01$`Pr(>F)`[1] > 0.05 &&
    levene_auto_02$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_position_geschlecht, "Faktorielle Mixed-ANOVA – Position × Geschlecht")
} else {
  zeige_wenn("robust_anova_position_geschlecht", "Robuste Mixed-ANOVA – Position × Geschlecht")
}
```
...
```{r geschlecht-groesse, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 2. Größe × Geschlecht 
...
```{r geschlecht-groesse-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_groesse_geschlecht, "Deskriptive Kennwerte – Größe × Geschlecht")
zeige_ergebnis(shapiro_auto_06, "Shapiro-Wilk-Test der Residuen – Größe × Geschlecht")
zeige_ergebnis(levene_auto_03, "Levene-Test – KDS nach Geschlecht")
zeige_ergebnis(levene_auto_04, "Levene-Test – GDS nach Geschlecht")
if (shapiro_auto_06$p.value > 0.05 &&
    levene_auto_03$`Pr(>F)`[1] > 0.05 &&
    levene_auto_04$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_groesse_geschlecht, "Faktorielle Mixed-ANOVA – Größe × Geschlecht")
} else {
  zeige_wenn("robust_anova_groesse_geschlecht", "Robuste Mixed-ANOVA – Größe × Geschlecht")
}
```

## Stärke (2 Stufen) × Geschlecht


```{r geschlecht-staerke, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 3. Stärke × Geschlecht 
...
```{r geschlecht-staerke-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke_geschlecht, "Deskriptive Kennwerte – Stärke (2 Stufen) × Geschlecht")
zeige_ergebnis(shapiro_auto_07, "Shapiro-Wilk-Test der Residuen – Stärke (2 Stufen) × Geschlecht")
zeige_ergebnis(levene_auto_05, "Levene-Test – GDL nach Geschlecht")
zeige_ergebnis(levene_auto_06, "Levene-Test – GDS nach Geschlecht")
if (shapiro_auto_07$p.value > 0.05 &&
    levene_auto_05$`Pr(>F)`[1] > 0.05 &&
    levene_auto_06$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_geschlecht, "Faktorielle Mixed-ANOVA – Stärke (2 Stufen) × Geschlecht")
} else {
  zeige_wenn("robust_anova_staerke_geschlecht", "Robuste Mixed-ANOVA – Stärke (2 Stufen) × Geschlecht")
}
```

## Schuh-Stärke (2 Stufen) × Geschlecht


```{r geschlecht-schuh-staerke, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r geschlecht-schuh-staerke-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  deskriptiv_staerke_schuh_geschlecht,
  "Deskriptive Kennwerte – Schuh-Stärke (2 Stufen) × Geschlecht"
)
zeige_ergebnis(shapiro_auto_08, "Shapiro-Wilk-Test der Residuen – Schuh-Stärke (2 Stufen) × Geschlecht")
zeige_ergebnis(levene_auto_07, "Levene-Test – M nach Geschlecht")
zeige_ergebnis(levene_auto_08, "Levene-Test – KDS nach Geschlecht")
if (shapiro_auto_08$p.value > 0.05 &&
    levene_auto_07$`Pr(>F)`[1] > 0.05 &&
    levene_auto_08$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(
  ...
  zeige_wenn(
    "deskriptiv_staerke_schuh_robust",
    "Robuste deskriptive Kennwerte – Schuh-Stärke (2 Stufen) × Geschlecht"
  )
  zeige_wenn(
    "robust_anova_staerke_schuh_geschlecht",
    "Robuste Mixed-ANOVA – Schuh-Stärke (2 Stufen) × Geschlecht"
  )
}
```

## Stärke (3 Stufen) × Geschlecht


```{r geschlecht-staerke-drei, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r geschlecht-staerke-drei-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke3_geschlecht, "Deskriptive Kennwerte – Stärke (3 Stufen) × Geschlecht")
zeige_ergebnis(shapiro_auto_09, "Shapiro-Wilk-Test der Residuen – Stärke (3 Stufen) × Geschlecht")
zeige_ergebnis(levene_auto_09, "Levene-Test – M nach Geschlecht")
zeige_ergebnis(levene_auto_10, "Levene-Test – GDL nach Geschlecht")
zeige_ergebnis(levene_auto_11, "Levene-Test – GDS nach Geschlecht")
zeige_ergebnis(summary_anova_staerke3_geschlecht, "ANOVA-Zusammenfassung einschließlich Sphärizitätsprüfung")
if (shapiro_auto_09$p.value > 0.05 &&
    levene_auto_09$`Pr(>F)`[1] > 0.05 &&
    levene_auto_10$`Pr(>F)`[1] > 0.05 &&
    levene_auto_11$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke3_geschlecht, "Faktorielle Mixed-ANOVA – Stärke (3 Stufen) × Geschlecht")
} else {
  zeige_wenn("robust_anova_staerke3_geschlecht", "Robuste Mixed-ANOVA – Stärke (3 Stufen) × Geschlecht")
}
```

# Unterschiede nach CVPA


## CVPA-Gruppen und Hilfsfunktionen


```{r cvpa-hilfsfunktionen, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# CVPA-Unterschiede
...
```{r cvpa-gruppen-tabelle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(
  cvpa_personen %>% count(CVPA_Gruppe),
  "Verteilung der CVPA-Gruppen"
)
zeige_ergebnis(
  data.frame(Kennwert = "Gesamtmittelwert CVPA", Wert = cvpa_gesamtmittelwert),
  "CVPA-Gesamtmittelwert"
)
```
...
```{r cvpa-position, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 1. Position × CVPA 
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Position × CVPA",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r cvpa-position-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_position_cvpa, "Deskriptive Kennwerte – Position × CVPA")
zeige_ergebnis(shapiro_auto_10, "Shapiro-Wilk-Test der Residuen – Position × CVPA")
zeige_ergebnis(levene_auto_12, "Levene-Test – GDS nach CVPA")
zeige_ergebnis(levene_auto_13, "Levene-Test – GZS nach CVPA")
if (shapiro_auto_10$p.value > 0.05 &&
    levene_auto_12$`Pr(>F)`[1] > 0.05 &&
    levene_auto_13$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_position_cvpa, "Faktorielle Mixed-ANOVA – Position × CVPA")
} else {
  zeige_wenn("robust_anova_position_cvpa", "Robuste Mixed-ANOVA – Position × CVPA")
}
```
...
```{r cvpa-groesse, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 2. Größe × CVPA 
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Größe × CVPA",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r cvpa-groesse-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_groesse_cvpa, "Deskriptive Kennwerte – Größe × CVPA")
zeige_ergebnis(shapiro_auto_12, "Shapiro-Wilk-Test der Residuen – Größe × CVPA")
zeige_ergebnis(levene_auto_16, "Levene-Test – KDS nach CVPA")
zeige_ergebnis(levene_auto_17, "Levene-Test – GDS nach CVPA")
if (shapiro_auto_12$p.value > 0.05 &&
    levene_auto_16$`Pr(>F)`[1] > 0.05 &&
    levene_auto_17$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_groesse_cvpa, "Faktorielle Mixed-ANOVA – Größe × CVPA")
} else {
  zeige_wenn("deskriptiv_groesse_cvpa_robust", "Robuste deskriptive Kennwerte – Größe × CVPA")
  zeige_wenn("robust_anova_groesse_cvpa", "Robuste Mixed-ANOVA – Größe × CVPA")
}
```

## Stärke (2 Stufen) × CVPA


```{r cvpa-staerke, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 3. Stärke × CVPA 
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Stärke (2 Stufen) × CVPA",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r cvpa-staerke-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke_cvpa, "Deskriptive Kennwerte – Stärke (2 Stufen) × CVPA")
zeige_ergebnis(shapiro_auto_13, "Shapiro-Wilk-Test der Residuen – Stärke (2 Stufen) × CVPA")
zeige_ergebnis(levene_auto_18, "Levene-Test – GDL nach CVPA")
zeige_ergebnis(levene_auto_19, "Levene-Test – GDS nach CVPA")
if (shapiro_auto_13$p.value > 0.05 &&
    levene_auto_18$`Pr(>F)`[1] > 0.05 &&
    levene_auto_19$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_cvpa, "Faktorielle Mixed-ANOVA – Stärke (2 Stufen) × CVPA")
} else {
  zeige_wenn("robust_anova_staerke_cvpa", "Robuste Mixed-ANOVA – Stärke (2 Stufen) × CVPA")
}
```

## Stärke (3 Stufen) × CVPA


```{r cvpa-staerke-drei, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Stärke (3 Stufen) × CVPA",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r cvpa-staerke-drei-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke3_cvpa, "Deskriptive Kennwerte – Stärke (3 Stufen) × CVPA")
zeige_ergebnis(shapiro_auto_14, "Shapiro-Wilk-Test der Residuen – Stärke (3 Stufen) × CVPA")
zeige_ergebnis(levene_auto_20, "Levene-Test – M nach CVPA")
zeige_ergebnis(levene_auto_21, "Levene-Test – GDL nach CVPA")
zeige_ergebnis(levene_auto_22, "Levene-Test – GDS nach CVPA")
zeige_ergebnis(summary_anova_staerke3_cvpa, "ANOVA-Zusammenfassung einschließlich Sphärizitätsprüfung – CVPA")
if (shapiro_auto_14$p.value > 0.05 &&
    levene_auto_20$`Pr(>F)`[1] > 0.05 &&
    levene_auto_21$`Pr(>F)`[1] > 0.05 &&
    levene_auto_22$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke3_cvpa, "Faktorielle Mixed-ANOVA – Stärke (3 Stufen) × CVPA")
} else {
  zeige_wenn("robust_anova_staerke3_cvpa", "Robuste Mixed-ANOVA – Stärke (3 Stufen) × CVPA")
}
```

## Schuh-Stärke (2 Stufen) × CVPA


```{r cvpa-schuh-staerke, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# 5. Stärke Schuh × CVPA 
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = stringr::str_wrap(
        "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Schuh-Stärke (2 Stufen) × CVPA",
        width = 52),
      x = "Residuen",
      y = "Dichte"
    ) +
...
```{r cvpa-schuh-staerke-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_staerke_schuh_cvpa, "Deskriptive Kennwerte – Schuh-Stärke (2 Stufen) × CVPA")
zeige_ergebnis(shapiro_auto_15, "Shapiro-Wilk-Test der Residuen – Schuh-Stärke (2 Stufen) × CVPA")
zeige_ergebnis(levene_auto_23, "Levene-Test – M nach CVPA")
zeige_ergebnis(levene_auto_24, "Levene-Test – KDS nach CVPA")
if (shapiro_auto_15$p.value > 0.05 &&
    levene_auto_23$`Pr(>F)`[1] > 0.05 &&
    levene_auto_24$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_schuh_cvpa, "Faktorielle Mixed-ANOVA – Schuh-Stärke (2 Stufen) × CVPA")
} else {
  zeige_wenn(
    "deskriptiv_staerke_schuh_cvpa_robust",
    "Robuste deskriptive Kennwerte – Schuh-Stärke (2 Stufen) × CVPA"
  )
  zeige_wenn("robust_anova_staerke_schuh_cvpa", "Robuste Mixed-ANOVA – Schuh-Stärke (2 Stufen) × CVPA")
}
```

# Ermüdungseffekt und Reihenfolge


Geprüft wird, ob ein Outfit unterschiedlich bewertet wird, wenn es als erstes oder als letztes gezeigt wurde.


```{r ermuedung, echo=TRUE, message=FALSE, warning=FALSE, results='hide', fig.height=5.8}
# Ermüdungseffekt 
...
# Normalverteilung grafisch darstellen: Density Plot (nur deskriptiv; nicht für die
# Testwahl)
zeige_und_speichere_grafik(
...
```{r ermuedung-tabellen, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(deskriptiv_ermuedung, "Deskriptive Kennwerte – erstes vs. letztes Outfit")
zeige_ergebnis(shapiro_ermuedung, "Shapiro-Wilk-Tests – Ermüdungsanalyse")
zeige_ergebnis(levene_ermuedung, "Levene-Tests – Ermüdungsanalyse")
zeige_ergebnis(ergebnisse_ermuedung, "Hypothesentests zum Ermüdungseffekt mit Holm-Korrektur")
```

# Mediationsanalyse


Die Mediationsanalyse folgt dem im Skript definierten vorlesungsnahen Ansatz. Der
Analysecode bleibt unverändert; die zusammengefassten Ergebnisse werden tabellarisch
ausgegeben.


```{r mediation, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r mediation-tabelle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(mediation_ergebnisse, "Ergebnisse der Mediationsanalysen")
```

# Ausblick: Wahrgenommene Stilzuordnung


```{r stilzuordnung, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Ausblick 
...
```{r stilzuordnung-tabelle, echo=FALSE, message=FALSE, warning=FALSE, results='asis'}
zeige_ergebnis(stilzuordnung_summary, "Mittelwerte und Standardfehler der wahrgenommenen Stilzuordnung")
```
