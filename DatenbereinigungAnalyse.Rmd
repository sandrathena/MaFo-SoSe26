...
Die Analyseberechnungen entsprechen dem zugrunde liegenden R-Skript. Änderungen in
dieser R-Markdown-Datei betreffen ausschließlich **Struktur, Überschriften und
Ergebnisdarstellung**. Rohe Konsolenausgaben der Analyse-Chunks werden unterdrückt;
relevante Ergebnisobjekte werden anschließend übersichtlich als Tabellen dargestellt.
Grafiken werden weiterhin direkt ausgegeben und zusätzlich wie im Originalcode
gespeichert. Für die PDF-Ausgabe werden lange Codezeilen automatisch umgebrochen, breite
Tabellen seitengerecht geteilt und Grafiken in normaler Berichtsgröße gesetzt.
...
## Stärke der stilistischen Abweichung: GDS vs. GDL
...
zeige_ergebnis(deskriptiv_staerke_zwei, "Deskriptive Kennwerte – Stärke")
zeige_ergebnis(shapiro_auto_03, "Shapiro-Wilk-Test der Differenzwerte – Stärke")
zeige_wenn("t_test_staerke", "Gerichteter gepaarter t-Test – Stärke")
zeige_wenn("wilcoxon_staerke", "Gerichteter Wilcoxon-Vorzeichen-Rang-Test – Stärke")
```

## Stärke mit drei Stufen: M vs. GDL vs. GDS
...
zeige_ergebnis(deskriptiv_staerke, "Deskriptive Kennwerte – drei Stärkestufen")
zeige_ergebnis(shapiro_auto_04, "Shapiro-Wilk-Test der ANOVA-Residuen – drei Stärkestufen")
if (shapiro_auto_04$p.value > 0.05) {
  zeige_ergebnis(anova_staerke, "Repeated-Measures-ANOVA – drei Stärkestufen")
  zeige_ergebnis(as.data.frame(paarweise_staerke), "Paarweise Vergleiche mit Holm-Korrektur")
} else {
  zeige_wenn("friedman_staerke", "Friedman-Test – drei Stärkestufen")
  zeige_ergebnis(paarweise_staerke, "Paarweise Wilcoxon-Tests mit Holm-Korrektur")
}
```

## Baseline vs. stark abweichender Schuh: M vs. KDS
...
zeige_ergebnis(deskriptiv_staerke_geschlecht, "Deskriptive Kennwerte – Stärke × Geschlecht")
zeige_ergebnis(shapiro_auto_07, "Shapiro-Wilk-Test der Residuen – Stärke × Geschlecht")
...
if (shapiro_auto_07$p.value > 0.05 &&
    levene_auto_05$`Pr(>F)`[1] > 0.05 &&
    levene_auto_06$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_geschlecht, "Faktorielle Mixed-ANOVA – Stärke × Geschlecht")
} else {
  zeige_wenn("robust_anova_staerke_geschlecht", "Robuste Mixed-ANOVA – Stärke × Geschlecht")
}
```

## Schuh-Stärke × Geschlecht
...
zeige_ergebnis(deskriptiv_staerke_schuh_geschlecht, "Deskriptive Kennwerte – Schuh-Stärke × Geschlecht")
zeige_ergebnis(shapiro_auto_08, "Shapiro-Wilk-Test der Residuen – Schuh-Stärke × Geschlecht")
zeige_ergebnis(levene_auto_07, "Levene-Test – M nach Geschlecht")
zeige_ergebnis(levene_auto_08, "Levene-Test – KDS nach Geschlecht")
if (shapiro_auto_08$p.value > 0.05 &&
    levene_auto_07$`Pr(>F)`[1] > 0.05 &&
    levene_auto_08$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_schuh_geschlecht, "Faktorielle Mixed-ANOVA – Schuh-Stärke × Geschlecht")
} else {
  zeige_wenn("deskriptiv_staerke_schuh_robust", "Robuste deskriptive Kennwerte – Schuh-Stärke × Geschlecht")
  zeige_wenn("robust_anova_staerke_schuh_geschlecht", "Robuste Mixed-ANOVA – Schuh-Stärke × Geschlecht")
}
```

## Stärke mit drei Stufen × Geschlecht
...
zeige_ergebnis(deskriptiv_staerke3_geschlecht, "Deskriptive Kennwerte – drei Stärkestufen × Geschlecht")
zeige_ergebnis(shapiro_auto_09, "Shapiro-Wilk-Test der Residuen – drei Stärkestufen × Geschlecht")
...
if (shapiro_auto_09$p.value > 0.05 &&
    ...
    levene_auto_10$`Pr(>F)`[1] > 0.05 &&
    levene_auto_11$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke3_geschlecht, "Faktorielle Mixed-ANOVA – drei Stärkestufen × Geschlecht")
} else {
  zeige_wenn("robust_anova_staerke3_geschlecht", "Robuste Mixed-ANOVA – drei Stärkestufen × Geschlecht")
}
```
...
zeige_ergebnis(deskriptiv_staerke_cvpa, "Deskriptive Kennwerte – Stärke × CVPA")
zeige_ergebnis(shapiro_auto_13, "Shapiro-Wilk-Test der Residuen – Stärke × CVPA")
zeige_ergebnis(levene_auto_18, "Levene-Test – GDL nach CVPA")
zeige_ergebnis(levene_auto_19, "Levene-Test – GDS nach CVPA")
if (shapiro_auto_13$p.value > 0.05 &&
    levene_auto_18$`Pr(>F)`[1] > 0.05 &&
    levene_auto_19$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_cvpa, "Faktorielle Mixed-ANOVA – Stärke × CVPA")
} else {
  zeige_wenn("robust_anova_staerke_cvpa", "Robuste Mixed-ANOVA – Stärke × CVPA")
}
```

## Stärke mit Baseline × CVPA
...
zeige_ergebnis(deskriptiv_staerke3_cvpa, "Deskriptive Kennwerte – drei Stärkestufen × CVPA")
zeige_ergebnis(shapiro_auto_14, "Shapiro-Wilk-Test der Residuen – drei Stärkestufen × CVPA")
...
if (shapiro_auto_14$p.value > 0.05 &&
    ...
    levene_auto_21$`Pr(>F)`[1] > 0.05 &&
    levene_auto_22$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke3_cvpa, "Faktorielle Mixed-ANOVA – drei Stärkestufen × CVPA")
} else {
  zeige_wenn("robust_anova_staerke3_cvpa", "Robuste Mixed-ANOVA – drei Stärkestufen × CVPA")
}
```
...
zeige_ergebnis(deskriptiv_staerke_schuh_cvpa, "Deskriptive Kennwerte – Schuh-Stärke × CVPA")
zeige_ergebnis(shapiro_auto_15, "Shapiro-Wilk-Test der Residuen – Schuh-Stärke × CVPA")
zeige_ergebnis(levene_auto_23, "Levene-Test – M nach CVPA")
zeige_ergebnis(levene_auto_24, "Levene-Test – KDS nach CVPA")
if (shapiro_auto_15$p.value > 0.05 &&
    levene_auto_23$`Pr(>F)`[1] > 0.05 &&
    levene_auto_24$`Pr(>F)`[1] > 0.05) {
  zeige_ergebnis(anova_staerke_schuh_cvpa, "Faktorielle Mixed-ANOVA – Schuh-Stärke × CVPA")
} else {
  zeige_wenn("deskriptiv_staerke_schuh_cvpa_robust", "Robuste deskriptive Kennwerte – Schuh-Stärke × CVPA")
  zeige_wenn("robust_anova_staerke_schuh_cvpa", "Robuste Mixed-ANOVA – Schuh-Stärke × CVPA")
}
```
