...
# 1. Extrahieren der zweiten Zeile als Header
new_header <- as.character(salienz[2, ])
print(new_header)

# 2. Entfernen der ersten beiden Zeilen (alte Header und neue Headerzeile)
salienz <- salienz[-c(1, 2), ]

# 3. der neuen Headerzeilen als Spaltennamen
colnames(salienz) <- new_header

# 4. Entfernen erster Zeile
salienz <- salienz[-c(1), ]

# 5. Zuweisung von unique col names
...
# Beginn Analyse
# 1. Ziel: Stichproben Demografie aller Teilnehmer - inklusive Handy User - zum Überblick verschaffen 
...
geschlecht_legende <- setNames(
  paste0(
    as.character(geschlecht_plot$Geschlecht_Label),
    ": ",
    geschlecht_plot$n,
    " (",
    round(geschlecht_plot$Prozent, 1),
    " %)"),
  as.character(geschlecht_plot$Geschlecht_Label)
)
...
zeige_und_speichere_grafik(
  ggplot(geschlecht_plot, aes(x = "", y = n, fill = Geschlecht_Label)) +
    ...
    coord_polar(theta = "y") +
    geom_text(
      data = geschlecht_plot %>% filter(Prozent >= 5),
...
# Kreisdiagramm
zeige_und_speichere_grafik(
  ggplot(taetigkeit_plot, aes(x = "", y = n, fill = Beruf_Label)) +
    geom_col(width = 1, color = "white") +
    coord_polar(theta = "y") +
    geom_text(
      aes(label = paste0(Beruf_Label, "\n", n, " (", round(Prozent, 1), " %)")),
      position = position_stack(vjust = 0.5),
      size = 3.8) +
...
# Kreisdiagramm für Verteilung der Endgeräte
zeige_und_speichere_grafik(
  ggplot(
    geraete_plot,
    aes(x = "", y = n, fill = Geraetekategorie)) +
    geom_col(width = 1, color = "white") +
    coord_polar(theta = "y") +
    geom_text(
      aes(label = paste0(Geraetekategorie, "\n", n, " (", round(Prozent, 1), " %)")),
      position = position_stack(vjust = 0.5),
      size = 3.8) +
    scale_fill_manual(values = c(
      "PC / Laptop" = "#CDAA7D",
...
# 2. Gesamtbearbeitungszeit filtern
...
# Bgeinn der Datenanalyse mit bereinigtem Datensatz 
# Altersverteilung 
...
# Ausreßer mit Id ? 32 genauer betrachten 
...
# Berechnung von Cronbach Alpha und Interpretation nach Faustregel nach (Blanz, 2015): mind. 0.8
...
# Alle Skalen sind konsistent Mittelwerte können gebildet werden!
...
plot_box_density(
  ...
  subtitle = "Stärke (2 Stufen): GDL vs. GDS"
)
...
plot_box_density(
  ...
  subtitle = "Stärke (3 Stufen): M vs. GDL vs. GDS"
)
...
plot_box_density(
  ...
  subtitle = "Schuh-Stärke (2 Stufen): M vs. KDS"
)
...
schaetze_mixed_anova <- function(data, faktor) {
  modell <- aov_ez(
    id = "number",
    dv = "Mittelwert_Inkonsistenz",
    within = faktor,
...
levene_stufe <- function(data, faktor, stufe) {
  test <- leveneTest(
...
boxplot_staerke_leicht <- erstelle_boxplot_geschlecht(
  ...
  "Stärke (2 Stufen) – leichte stilistische Abweichung: GDL"
)
...
boxplot_staerke_stark <- erstelle_boxplot_geschlecht(
  ...
  "Stärke (2 Stufen) – starke stilistische Abweichung: GDS"
)
...
zeige_und_speichere_grafik(
  density_residuen(
    ...
    "Faktorielle Mixed-ANOVA: Stärke (2 Stufen) × Geschlecht",
    farbig = TRUE,
    zentriert = TRUE
  ),
  "Dichteplot der Residuen"
...
boxplot_staerke_schuh_baseline <- erstelle_boxplot_geschlecht(
  ...
  "Schuh-Stärke (2 Stufen) – Baseline: M"
)
...
boxplot_staerke_schuh_stark <- erstelle_boxplot_geschlecht(
  ...
  "Schuh-Stärke (2 Stufen) – stark: KDS"
)
...
zeige_und_speichere_grafik(
  density_residuen(
    ...
    "Faktorielle Mixed-ANOVA: Schuh-Stärke (2 Stufen) × Geschlecht"
  ),
  "Dichteplot der Residuen"
)
...
boxplot_staerke3_leicht <- erstelle_boxplot_geschlecht(
  ...
  "Stärke (3 Stufen) – leichte stilistische Abweichung: GDL"
)
...
boxplot_staerke3_stark <- erstelle_boxplot_geschlecht(
  ...
  "Stärke (3 Stufen) – starke stilistische Abweichung: GDS"
)
...
zeige_und_speichere_grafik(
  density_residuen(
    ...
    "Faktorielle Mixed-ANOVA: Stärke (3 Stufen) × Geschlecht"
  ),
  "Dichteplot der Residuen"
)
...
schaetze_cvpa_anova <- function(data, faktor) {
  modell <- aov_ez(
    id = "number",
    dv = "Mittelwert_Inkonsistenz",
    within = faktor,
...
shapiro_residuen_cvpa <- function(residuen) {
  test <- shapiro.test(as.numeric(residuen))
  test
}

levene_cvpa <- function(data, faktor, stufe) {
  test <- leveneTest(
...
levene_auto_12 <- levene_cvpa(
  inkonsistenz_position_cvpa, "Position", "dezentral"
)
levene_auto_12

levene_auto_13 <- levene_cvpa(
  inkonsistenz_position_cvpa, "Position", "zentral"
)
levene_auto_13
...
levene_auto_14 <- levene_cvpa(
  inkonsistenz_groesse_cvpa, "Groesse", "klein"
)
levene_auto_14

levene_auto_15 <- levene_cvpa(
  inkonsistenz_groesse_cvpa, "Groesse", "gross"
)
levene_auto_15
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Stärke (2 Stufen) × CVPA",
      x = "Residuen",
      y = "Dichte"
    ) +
    theme_box_density_gross +
...
levene_auto_18 <- levene_cvpa(
  inkonsistenz_staerke_cvpa, "Staerke", "leicht"
)
levene_auto_18

levene_auto_19 <- levene_cvpa(
  inkonsistenz_staerke_cvpa, "Staerke", "stark"
)
levene_auto_19
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    scale_y_continuous(breaks = 1:5, limits = c(1, 5)) +
    labs(
      title = "WSI nach Stärke (3 Stufen) und CVPA",
      subtitle = "3 Stufen: M, GDL und GDS",
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Stärke (3 Stufen) × CVPA",
      x = "Residuen",
      y = "Dichte"
    ) +
    theme_box_density_gross +
...
levene_auto_20 <- levene_cvpa(
  inkonsistenz_staerke3_cvpa, "Staerke", "baseline"
)
levene_auto_20

levene_auto_21 <- levene_cvpa(
  inkonsistenz_staerke3_cvpa, "Staerke", "leicht"
)
levene_auto_21

levene_auto_22 <- levene_cvpa(
  inkonsistenz_staerke3_cvpa, "Staerke", "stark"
)
levene_auto_22
...
zeige_und_speichere_grafik(
  ggplot(
    ...
    labs(
      title = "Dichteplot der Residuen: Faktorielle Mixed-ANOVA Schuh-Stärke (2 Stufen) × CVPA",
      x = "Residuen",
      y = "Dichte"
    ) +
    theme_box_density_gross +
...
levene_auto_23 <- levene_cvpa(
  inkonsistenz_staerke_schuh_cvpa, "Staerke", "baseline"
)
levene_auto_23

levene_auto_24 <- levene_cvpa(
  inkonsistenz_staerke_schuh_cvpa, "Staerke", "stark"
)
levene_auto_24
